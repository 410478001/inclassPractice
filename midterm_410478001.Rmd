...
```{r ans1b5}
hwGrades$Exercise3[which(hwGrades$`學號`=="4104d245c7")]
#hwGrades$Exercise3[which(classSurvey$`學號`=="4104d245c7")]
#which()得到的是"4104d245c7"在classSurvey中第幾行，套至hwGrades時行數不同
```
...
```{r ans132}
subsample<-classSurvey[which(classSurvey$`性別`=="女"),]
```
...
```{r ans133}
sum(str_detect(subsample$`課外活動`,"團隊活動"))/nrow(subsample)
```
...
```{r ans14}
hwGrades$`交作業次數`<-(1-is.na(hwGrades$hw1))+(1-is.na(hwGrades$hw2))+(1-is.na(hwGrades$Exercise3))
hwGrades
```
...
```{r ans15}
a<-replace(hwGrades,is.na(hwGrades),0)
hwGrades$`平均成績`<-(a$hw1+a$hw2+a$Exercise3)/3
hwGrades
```
...
```{r ans16}
NewhwGrade<-hwGrades[hwGrades$`學號`%in%classSurvey$`學號`,]
NewhwGrade
```
...
```{r ans17}
NewhwGrade$`女性`<-NewhwGrade$`學號`%in%classSurvey$`學號`[classSurvey$`性別`=="女"]
NewhwGrade
```
...
```{r ans18}
mean(NewhwGrade$`平均成績`[NewhwGrade$`女性`==FALSE])
```
...
```{r ans19}
mean(NewhwGrade[NewhwGrade$`學號`%in%subsample[which(str_detect(subsample$`課外活動`,"團隊活動")),]$`學號`,]$`平均成績`)
```
...
```{r ans21}
hwSubmit$`學號`<-as.factor(hwSubmit$`學號`)
