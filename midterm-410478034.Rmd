...
# method 2
# table(classSurvey$性別)/length(classSurvey$性別)
```
...
```{r ans133}
a<-sum(str_detect(subsample$課外活動, "團隊活動")) / length(subsample$課外活動)
b<-1-(sum(str_detect(subsample$課外活動, "團隊活動")) / length(subsample$課外活動))
c(a,b)

# method 2
# subsample$課外活動 %>%
  # str_detect(. ,"團隊活動") %>%
  # table(.) / length(subsample$課外活動)
```
...
```{r ans14}
hwGrades$交作業次數 <- rowSums(!is.na(hwGrades[ ,c(2:4)]))
```
...
```{r ans15}
hwGrades[ ,2:4] <- replace(hwGrades[ ,2:4], is.na(hwGrades[ ,2:4]), 0)
hwGrades$平均成績 <- rowMeans(hwGrades[,2:4])
hwGrades
```
...
```{r ans16}
hwGradeSubsample <- hwGrades[which(hwGrades$學號 %in% classSurvey$學號), ]
hwGradeSubsample
```
...
```{r ans18}
male <- which(hwGradeSubsample$女性 == FALSE)
colMeans(hwGradeSubsample[male,6])
```
...
```{r ans19}
femaleAct <- classSurvey$學號[str_detect(subsample$課外活動, "團隊活動")]
mean(hwGradeSubsample$平均成績[which(hwGradeSubsample$學號 %in% femaleAct)])
```
