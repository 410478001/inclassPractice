...
```{r ans11}
subsample<-transcriptData[which(transcriptData$學號=='edfeQ7fb8'),]
subsample
```
...
```{r ans12}
subsample<-transcriptData[which(transcriptData$'學號'=='edfeQ7fb8'),]
subsample%>%mutate(及格=學期成績>= 60)->subsample
summarise(subsample,'總修習學分數'=sum(subsample$'學分數'),
...
```{r ans13}
summarise(subsample,學號='edfeQ7fb8','總修習學分數'=sum(subsample$'學分數'),
'總實得學分數'=sum(subsample$學分數[subsample$及格]))
```
...
totalCredits <- function(x){
  newsample<-transcriptData[which(transcriptData$'學號'==x),]
  newsample%>%mutate(及格=學期成績>= 60)->newsample
  summarise(newsample,學號=x,'總修習學分數'=sum(newsample$'學分數'),
'總實得學分數'=sum(newsample$學分數[newsample$及格]))->t
  paste0(t)
}
...
```{r ans15}
transcriptData%>%mutate(系別=str_sub(transcriptData$學號,5,6))->transcriptData
```
...
```{r ans16}
transcriptData[which(transcriptData$學年==100&transcriptData$學期==1),]->newsample1
newsample1[newsample1$`必選修類別（必∕選∕通）`=="通",]->newsample1
unique(newsample1$科目名稱)
```
...
```{r ans17}
shannon<-function(x){
  p<-nrow(newsample2[which(newsample2$系別==x),])/nrow(newsample2)
  -sum(p*log(p))
}
shannon1<-function(x){
  nrow(newsample2[which(newsample2$系別==x),])
}
shannon1(c("P1","W5"))
sum(which(newsample2$系別==c("P1")))
```
...
```{r ans18}
newsample2<-transcriptData[which(transcriptData$學年==104&transcriptData$學期==1),]
newsample2[newsample2$科目名稱=="ZT13",]->newsample2
R<-unique(newsample2$系別)
shannon(R)
```
